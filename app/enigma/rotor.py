from .configuration import e_rotors
...
class Rotor:
    ...
    def __init__(self, wiring: e_rotors) -> None:
        ...
        match wiring:
            case e_rotors.I:
                self.right = e_rotors.I.value[0]
                self.notch = e_rotors.I.value[1]
            case e_rotors.II:
                self.right = e_rotors.II.value[0]
                self.notch = e_rotors.II.value[1]
            case e_rotors.III:
                self.right = e_rotors.III.value[0]
                self.notch = e_rotors.III.value[1]
            case e_rotors.IV:
                self.right = e_rotors.IV.value[0]
                self.notch = e_rotors.IV.value[1]
            case e_rotors.V:
                self.right = e_rotors.V.value[0]
                self.notch = e_rotors.V.value[1]
    ...
    def step(self, n: int = 1, forward: bool = True):
        for i in range(n):
            if forward:
    ...
    def set_ring(self, n: int):
        """Sets the starting position of the rotor ring. 1 starts at the default letter."""
        # rotate the rotor backwards
        self.step(n - 1, forward=False)
